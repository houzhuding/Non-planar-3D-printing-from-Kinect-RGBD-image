"""In situ bioprinting control
	...
	Author: Houzhu Ding
	This is Bioprinter Control GUI V7.0
"""
...
class BioprinterControl():
    def __init__(self,master):
        ...
        Label(master, text="IMAGE GUIDED BIOPRINTING CONTROL PANEL Ver7.0").grid(row=0,columnspan=10)
...
#################### HSV threshoding  ##############################     
        self.sliderbarKmean = Scale(master,from_=1,to=6,orient=HORIZONTAL,label='K for Kmean')
        self.sliderbarKmean.grid(row=6,column=1,columnspan=1)
        self.sliderbarKmean.set(2)
        self.sliderbarScale = Scale(master,from_=0.5, to=1.5,orient=HORIZONTAL,
            label='scale factor',resolution = 0.1)
        self.sliderbarScale.grid(row =6,column = 2,columnspan = 1)
        self.sliderbarScale.set(1)
    ...
    def LiveCamera(self,camNum,cap,canvas_):
        ...
        while(self.closeCamSign):  
            ...
            if camNum == 0 and self.img_crop is not None:
                ...
                roi_out,needle_loc_trans,contour_out,rect = self.ImgProcessTool.ContourExtractArUcoMarker(self.img_crop,self.show_botton,self.needle_location)
...
############################### 3D printing part ###################################
    ...
    def DrawDanamicToolPath(self):
        ...
        self.flowRates = [self.flowrate]*len(self.path)
        self.flowRates[0] = 0
        self.flowRates[-1] = 0
    ...
    def ShowStlModel(self,):
        ...
        #     # define the default countour of the ROI
        #     xmin = -12
        #     xmax = 8
        #     ymin = -10
        #     ymax = 10
        #     xy_region = [[xmin,ymin],[xmax,ymin],[xmax,ymax],[xmin,ymax],[xmin,ymin]] ## [[xmin,xmax],[ymin,ymax]]
        #     # xy_region = [[xmin,ymin],[xmax,ymin],[xmin,ymax]] 
        # else:
        #     xy_region = self.contour

        # fileName = 'surface3D.nc'
        # toolPathObj = toolpathGenerationFromContour3D(np.array(xy_region),hand_mesh,step,feed)
        # key_pts,X,Y,Z = toolPathObj.scanHorizental()
        # toolPathObj.contour2path(key_pts,fileName,)

        # fig = plt.figure()
        # ax = fig.add_subplot(111, projection='3d')
    def ExtractContour(self,): ## Extract the contour and generate toolpath
        self.pathFile = 'test.nc'
        print (self.pathFile)
        ...
        # plot 3d mesh toolpath
        hand_mesh = self.ShowStlModel()
        ...
        toolPathObj3D = toolpathGenerationFromContour3D(np.array(xy_region),hand_mesh,step,feed)
        key_pts,X,Y,Z = toolPathObj3D.scanHorizental()
        # print(key_pts)
        key_pts = 1.1*160*key_pts/0.16
        minz = min(key_pts[:,2])
        key_pts[:,2] -= minz
        key_pts[:,[0, 1]] = key_pts[:,[1, 0]]
        key_pts[:,0] += 14/0.16
        key_pts[:,1] -= -0/0.16
        toolPathObj3D.contour2path(key_pts,self.pathFile)
        self.a.plot(key_pts[:,0],key_pts[:,1],key_pts[:,2])

        ### Extract contour and generate toolpath ###
        ...
        if len(self.contour)>2:
            ...
            real_scale_factor = 1.0/scale_img2real # 12.84 pixels = 1mm (real) = 1/6.2 mm (machine)
            cnt_real_scale = self.contour*real_scale_factor
            # print cnt_real_scale
            # self.a.plot(cnt_real_scale[:,0]-320*real_scale_factor-offsetX,-cnt_real_scale[:,1]+240*real_scale_factor-offsetY)
        else:
            print("No contour!")
            return

        # print(cnt_real_scale)
        z_path = len(cnt_real_scale)*[0]
        ...
        self.path = np.transpose(np.array([-cnt_real_scale[:,1],cnt_real_scale[:,0],z_path]))
        # print path  
        self.a.plot(self.path[:,0],-self.path[:,1],self.path[:,2])
        self.a.scatter(0,0)
        self.a.set_xlabel('X direction /mm')
        self.a.set_ylabel('Y direction /mm')
        self.a.set_title('Toolpath planning')
        # self.a.set_aspect('equal', 'box')
        xmin = min(self.path[:,0])-20;xmax = max(self.path[:,0])+50;
        ymin = min(self.path[:,1])-10;ymax = max(self.path[:,1])+10
        self.a.set_xlim([xmin,xmax])
        self.a.set_ylim([ymin,ymax])
        # self.cntPlot.get_tk_widget().pack(side=TOP, fill=BOTH, expand=1)
        self.cntPlot.draw()
        # convert path to gcode by scaling real scale to machine scale

        # machine_scale_factor = 1/6.2
        machine_scale_factor =1.1# 10/6.1/1.6
        cnt_machine_scale = self.path*machine_scale_factor
        final_pts = np.concatenate((cnt_machine_scale,key_pts),axis=0)
        toolPathObj.contour2path(cnt_machine_scale,'test.nc')
